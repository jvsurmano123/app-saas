...
interface NewConsultationPageClientProps {
  patientId: string;
}

export function NewConsultationPageClient({ patientId }: NewConsultationPageClientProps) {
  ...
  const handleSubmit = async (data: any) => {
    try {
      ...
      router.push(`/dashboard/medical-records/${patientId}`);
    } catch (error) {
  ...
  return (
    <div className="container py-6 max-w-3xl">
      ...
      <NewConsultationForm patientId={patientId} onSubmit={handleSubmit} />
    </div>
  );
} 